...
const FloatingShapes = ({ className = "absolute inset-0" }: FloatingShapesProps) => {
  return (
    <div className={`${className} pointer-events-none overflow-hidden z-0`}>
      {/* 3D Octahedron CSS Shape - Variation 1 */}
      <motion.div
        animate={{
          rotateY: [0, 360],
          rotateX: [0, 180, 0],
      ...
      {/* 3D Octahedron CSS Shape - Variation 2 (Bottom Left) */}
      <motion.div
        animate={{
          rotateY: [360, 0],
          rotateZ: [0, 90, 0],
        ...
        className="absolute bottom-20 left-[5%] w-8 h-8 md:w-16 md:h-16 opacity-[0.07]"
        style={{ transformStyle: "preserve-3d" }}
      >
        <div className="relative w-full h-full" style={{ transformStyle: "preserve-3d" }}>
          {[0, 90, 180, 270].map((rotate, i) => (
            <div
              key={i}
              className="absolute w-0 h-0 border-l-[32px] border-l-transparent border-r-[32px] border-r-transparent border-b-[55px] border-b-primary/40"
              style={{
                transform: `rotateY(${rotate}deg) translateZ(0px) rotateX(35deg)`,
                transformOrigin: "50% 100%",
              }}
          ...
          {[0, 90, 180, 270].map((rotate, i) => (
            <div
              key={i + 4}
              className="absolute w-0 h-0 border-l-[32px] border-l-transparent border-r-[32px] border-r-transparent border-t-[55px] border-t-primary/20"
              style={{
                transform: `rotateY(${rotate}deg) translateZ(0px) rotateX(-35deg)`,
                transformOrigin: "50% 0%",
                top: "55px",
