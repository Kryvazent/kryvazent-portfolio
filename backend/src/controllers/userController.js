...
export const createUser = async (request, response) => {
  const { name, email, password, role = "editor", team = "maintenance" } = request.body || {};
  ...
  const user = await User.create({ name: String(name).trim(), email: String(email).toLowerCase().trim(), passwordHash: await bcrypt.hash(password, 12), role, team });
...
export const updateUser = async (request, response) => {
  ...
  const { name, email, password, role, team, active } = request.body || {};
