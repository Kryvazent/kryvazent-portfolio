...
export default function ClientProjectsPage() {
  ...
  if (selected) return <section>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      ...
      <ProjectCollection title="Project links" items={selected.links.map((item) => <a key={item._id} href={item.url} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-xl border border-line p-4 hover:border-primary"><span><strong className="block">{item.label}</strong><span className="text-xs capitalize text-muted">{item.category}</span></span><ExternalLink className="h-4 w-4" /></a>)}><div className="grid gap-3"><Field label="Link label" value={newLink.label} onChange={(label) => setNewLink({ ...newLink, label })} /><Field label="URL" value={newLink.url} onChange={(url) => setNewLink({ ...newLink, url })} /><button onClick={() => void add("links", newLink).then(() => setNewLink({ label: "", url: "", category: "other" }))} className="rounded-xl bg-primary px-4 py-3 text-sm font-bold text-white">Add link</button></div></ProjectCollection>
    </div>
  </section>;
